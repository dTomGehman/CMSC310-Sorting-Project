...
double process_file(int n_data, int file_no, char*unssorrev,
        char*dir, char*salgo, short*buffer, int prince);
...
int main(int argc, char**argv){
    ...
    if (argc != 7 && argc != 8) {
        fprintf(stderr, "Usage: "
                ...
                " \n\t\t\t<file_end> <selection|merge|quick> [print]\n"
                ...
                "\tspecify print option to print out the buffer before/after sort\n"
    ...
    char*salgo = argv[6];
    int prince=0;
    if (argc==8 && !strcmp(argv[7], "print")) prince=1;
    ...
    for (int i=file_start; i<=file_end; i++)
        printf(" %lf\n", process_file(n_data, i, unssorrev, dir, salgo, buffer, prince));
